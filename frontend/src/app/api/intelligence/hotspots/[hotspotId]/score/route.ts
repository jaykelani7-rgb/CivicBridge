...
export async function GET(request: NextRequest, context: { params: Promise<{ hotspotId: string }> }) {
  try {
    ...
    const result = await cloudRunRequest<Record<string, unknown>>({ service: "intelligence", path: `/v1/hotspots/${encodeURIComponent(hotspotId)}/score`, signal: request.signal });
